...
pub fn write_cycles(name: &str, cycles: u64) {
    let Some(file) = std::env::var("MONEROCHAN_PATCH_BENCH").ok().map(std::path::PathBuf::from) else {
        return;
    };
...
lazy_static::lazy_static! {
    ...
    pub static ref MONEROCHAN_CPU_PROVER: monerochan::cpu::CpuProver = monerochan::cpu::CpuProver::new();
}
