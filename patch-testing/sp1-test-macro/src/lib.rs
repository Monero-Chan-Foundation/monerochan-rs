...
/// The `monerochan_test` attribute is used to define a test case one time, that can be used to test
...
/// ```rust,ignore
/// fn test_name(stdin: &mut monerochan::MONEROCHANStdin) -> impl FnOnce(MONEROCHANPublicValues);
/// ```
///
/// The body of `test_name` is the "setup", and the returned `FnOnce(MONEROCHANPublicValues)` is the
/// "check".
///
/// Here is a full example of how to use this macro:
/// ```rust,ignore
/// #[monerochan_test("sha_256_program")]
/// fn test_expected_digest_rand_times_lte_100_test(stdin: &mut monerochan::MONEROCHANStdin) -> impl FnOnce(MONEROCHANPublicValues) {
...
/// ```
///
/// Note: You MUST have monerochan in your dependencies, and you MUST have and to use the `gpu`
...
pub fn monerochan_test(attr: TokenStream, item: TokenStream) -> TokenStream {
    if attr.is_empty() {
        return syn::Error::new_spanned(
            ...
            "The MONEROCHAN test attribute requires at least an ELF file to be specified",
        )
        .to_compile_error()
        .into();
    }
    ...
    if matches!(setup_fn.sig.output, syn::ReturnType::Default) {
        return syn::Error::new_spanned(
            &setup_fn.sig,
            "The MONEROCHAN test attribute requires a return type: `impl FnOnce(monerochan::MONEROCHANPublicValues)",
        )
        .to_compile_error()
        .into();
    }
    ...
    let elf_name = match options.elf_name() {
        Some(elf) => elf,
        None => panic!("The MONEROCHAN test attribute requires an ELF file to be specified"),
    };
    ...
    let maybe_client_setup = options.setup().map(|setup| {
        quote! {
            fn __asset_proper_setup<F: FnOnce(::monerochan::CpuProver) -> ::monerochan::ProverClient>(setup: F) {
                let _ = setup;
            }
    ...
    let gpu_prove = if options.gpu() {
        ...
        Some(quote! {
            ...
            fn #gpu_prove_name() {
                ...
                let _lock = ::monerochan_test::lock_serial();

                // Note: We must sleep on gpu tests to wait for Docker cleanup.
                std::thread::sleep(std::time::Duration::from_secs(5));

                let __macro_internal_client = ::monerochan::ProverClient::builder().cuda().build();
                let mut __macro_internal_stdin = ::monerochan::MONEROCHANStdin::new();
