...
pub(crate) fn create_local_command(
    ...
    // downloaded by 'monerochanup --c-toolchain'.
    if env::var("CC_riscv32im_succinct_zkvm_elf").is_err() {
        if let Some(home_dir) = home_dir() {
            let cc_path = home_dir.join(".monerochan").join("bin").join("riscv32-unknown-elf-gcc");
            if cc_path.exists() {
    ...
    let parsed_version = {
        ...
        if matches!(args.warning_level, WarningLevel::All) {
            println!("cargo:warning=rustc +monerochan --version: {stdout_string:?}");
        }
    ...
    //    program with the toolchain of the normal build process, rather than the Monerochan Foundation
    //    toolchain.
