...
/// Uses MONEROCHAN_DOCKER_IMAGE environment variable if set, otherwise constructs the image to use based
/// on the provided tag.
fn get_docker_image(tag: &str) -> String {
    std::env::var("MONEROCHAN_DOCKER_IMAGE").unwrap_or_else(|_| {
        let image_base = "ghcr.io/monero-chan-foundation/monerochan-rs";
...
pub(crate) fn create_docker_command(
    ...
    let parsed_version = {
        ...
        if matches!(args.warning_level, WarningLevel::All) {
            println!("cargo:warning=docker: rustc +monerochan --version: {stdout_string:?}");
        }
    ...
    // Add the MONEROCHAN program build arguments.
