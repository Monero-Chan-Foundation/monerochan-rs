...
pub(crate) fn build_program_internal(path: &str, args: Option<BuildArgs>) {
    ...
    // Skip the program build if the MONEROCHAN_SKIP_PROGRAM_BUILD environment variable is set to true.
    let skip_program_build = std::env::var("MONEROCHAN_SKIP_PROGRAM_BUILD")
    ...
    if skip_program_build {
        ...
        println!(
            "cargo:warning=Build skipped for {} at {} due to MONEROCHAN_SKIP_PROGRAM_BUILD flag",
            root_package_name,
            current_datetime()
        );
        return;
    ...
    // current compiler). If so, don't execute `cargo monerochan build` because it breaks
    ...
    if let Err(err) = path_output {
        panic!("Failed to build MONEROCHAN program: {err}.");
    }
...
/// Prints cargo directives setting relevant `MONEROCHAN_ELF_` environment variables.
