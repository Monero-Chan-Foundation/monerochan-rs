...
pub struct BuildArgs {
    ...
    #[arg(
        long,
        help = "The ghcr.io/monero-chan-foundation/monerochan-rs image tag to use when building with Docker.",
...
///
/// Set the `MONEROCHAN_SKIP_PROGRAM_BUILD` environment variable to `true` to skip building the program.
...
///
/// Note that this only works when using `monerochan_build::build_program` or
/// `monerochan_build::build_program_with_args` in a build script.
///
