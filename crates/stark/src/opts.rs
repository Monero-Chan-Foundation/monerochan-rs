...
/// Options to configure the MONEROCHAN prover for core and recursive proofs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct MONEROCHANProverOpts {
    /// Options for the core prover.
    pub core_opts: MONEROCHANCoreOpts,
    /// Options for the recursion prover.
    pub recursion_opts: MONEROCHANCoreOpts,
}

impl MONEROCHANProverOpts {
    /// Get the default prover options.
    #[must_use]
    pub fn auto() -> Self {
        let cpu_ram_gb = System::new_all().total_memory() / (1024 * 1024 * 1024);
        MONEROCHANProverOpts::cpu(cpu_ram_gb as usize)
    }
    ...
    pub fn cpu(cpu_ram_gb: usize) -> Self {
        ...
        let mut opts = MONEROCHANProverOpts::default();
...
impl Default for MONEROCHANProverOpts {
    fn default() -> Self {
        Self { core_opts: MONEROCHANCoreOpts::default(), recursion_opts: MONEROCHANCoreOpts::recursion() }
    }
}

impl Default for MONEROCHANCoreOpts {
    fn default() -> Self {
        let cpu_ram_gb = System::new_all().total_memory() / (1024 * 1024 * 1024);
        let (default_log2_shard_size, default_shard_batch_size, default_log2_divisor) =
            MONEROCHANProverOpts::get_memory_opts(cpu_ram_gb as usize);
...
mod tests {
    ...
    #[test]
    fn test_opts() {
        let opts = MONEROCHANProverOpts::cpu(8);
        println!("8: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(15);
        println!("15: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(16);
        println!("16: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(32);
        println!("32: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(36);
        println!("36: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(64);
        println!("64: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(128);
        println!("128: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(256);
        println!("256: {:?}", opts.core_opts);

        let opts = MONEROCHANProverOpts::cpu(512);
