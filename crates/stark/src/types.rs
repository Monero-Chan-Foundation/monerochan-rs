...
/// The maximum number of elements that can be stored in the public values vec.  Both MONEROCHAN and
...
/// A proof generated by the MONEROCHAN RISC-V zkVM.
...
#[strum_discriminants(name(MONEROCHANProofMode))]
pub enum MONEROCHANProof {
    ...
    Compressed(Box<MONEROCHANReduceProof<InnerSC>>),
...
impl core::fmt::Display for MONEROCHANProof {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            MONEROCHANProof::Core(_) => write!(f, "Core"),
            MONEROCHANProof::Compressed(_) => write!(f, "Compressed"),
            MONEROCHANProof::Plonk(_) => write!(f, "Plonk"),
            MONEROCHANProof::Groth16(_) => write!(f, "Groth16"),
        }
    }
}
...
pub struct MONEROCHANReduceProof<SC: StarkGenericConfig> {
...
impl<SC: StarkGenericConfig> std::fmt::Debug for MONEROCHANReduceProof<SC> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut debug_struct = f.debug_struct("MONEROCHANReduceProof");
