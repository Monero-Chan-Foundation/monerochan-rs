...
use crate::air::MONEROCHANAirBuilder;
...
impl<T> Word<T> {
    ...
    pub fn extend_var<AB: MONEROCHANAirBuilder<Var = T>>(var: T) -> Word<AB::Expr> {
...
impl<T: AbstractField> Word<T> {
    /// Extends a variable to a word.
    pub fn extend_expr<AB: MONEROCHANAirBuilder<Expr = T>>(expr: T) -> Word<AB::Expr> {
        Word([AB::Expr::zero() + expr, AB::Expr::zero(), AB::Expr::zero(), AB::Expr::zero()])
    }

    /// Returns a word with all zero expressions.
    #[must_use]
    pub fn zero<AB: MONEROCHANAirBuilder<Expr = T>>() -> Word<T> {
