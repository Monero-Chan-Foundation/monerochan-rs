...
use crate::{
    ...
    Groth16Bn254Proof, MONEROCHAN_CIRCUIT_VERSION,
};
...
impl Groth16Bn254Prover {
    ...
    pub fn build_contracts(build_dir: PathBuf) {
        ...
        let monerochan_verifier_path = build_dir.join("MONEROCHANVerifierGroth16.sol");
        let vkey_hash = Self::get_vkey_hash(&build_dir);
        let monerochan_verifier_str = include_str!("../assets/MONEROCHANVerifierGroth16.txt")
            .replace("{MONEROCHAN_CIRCUIT_VERSION}", MONEROCHAN_CIRCUIT_VERSION)
            .replace("{VERIFIER_HASH}", format!("0x{}", hex::encode(vkey_hash)).as_str())
            .replace("{PROOF_SYSTEM}", "Groth16");
        fs::write(monerochan_verifier_path, monerochan_verifier_str).unwrap();
    }
