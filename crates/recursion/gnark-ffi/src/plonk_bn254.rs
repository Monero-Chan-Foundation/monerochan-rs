...
use crate::{
    ...
    PlonkBn254Proof, MONEROCHAN_CIRCUIT_VERSION,
};
use anyhow::Result;
...
impl PlonkBn254Prover {
    ...
    pub fn build<C: Config>(constraints: Vec<Constraint>, witness: Witness<C>, build_dir: PathBuf) {
        ...
        let monerochan_verifier_path = build_dir.join("MONEROCHANVerifierPlonk.sol");
        let vkey_hash = Self::get_vkey_hash(&build_dir);
        let monerochan_verifier_str = include_str!("../assets/MONEROCHANVerifierPlonk.txt")
