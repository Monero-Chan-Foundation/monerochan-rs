...
import (
	...
	"github.com/monero-chan-foundation/monerochan-rs-recursion-gnark/monerochan"
	"github.com/monero-chan-foundation/monerochan-rs-recursion-gnark/monerochan/babybear"
	"github.com/monero-chan-foundation/monerochan-rs-recursion-gnark/monerochan/poseidon2"
)
...
func ProvePlonkBn254(dataDir *C.char, witnessPath *C.char) *C.C_PlonkBn254Proof {
	...
	monerochanPlonkBn254Proof := monerochan.ProvePlonk(dataDirString, witnessPathString)
	...
	structPtr.PublicInputs[0] = C.CString(monerochanPlonkBn254Proof.PublicInputs[0])
	structPtr.PublicInputs[1] = C.CString(monerochanPlonkBn254Proof.PublicInputs[1])
	structPtr.EncodedProof = C.CString(monerochanPlonkBn254Proof.EncodedProof)
	structPtr.RawProof = C.CString(monerochanPlonkBn254Proof.RawProof)
	return structPtr
}
...
func BuildPlonkBn254(dataDir *C.char) {
	...
	monerochan.BuildPlonk(dataDirString)
}
...
func VerifyPlonkBn254(dataDir *C.char, proof *C.char, vkeyHash *C.char, committedValuesDigest *C.char) *C.char {
	...
	err := monerochan.VerifyPlonk(dataDirString, proofString, vkeyHashString, committedValuesDigestString)
	if err != nil {
...
func ProveGroth16Bn254(dataDir *C.char, witnessPath *C.char) *C.C_Groth16Bn254Proof {
	...
	monerochanGroth16Bn254Proof := monerochan.ProveGroth16(dataDirString, witnessPathString)
	...
	structPtr.PublicInputs[0] = C.CString(monerochanGroth16Bn254Proof.PublicInputs[0])
	structPtr.PublicInputs[1] = C.CString(monerochanGroth16Bn254Proof.PublicInputs[1])
	structPtr.EncodedProof = C.CString(monerochanGroth16Bn254Proof.EncodedProof)
	structPtr.RawProof = C.CString(monerochanGroth16Bn254Proof.RawProof)
	return structPtr
}
...
func BuildGroth16Bn254(dataDir *C.char) {
	...
	monerochan.BuildGroth16(dataDirString)
}
...
func VerifyGroth16Bn254(dataDir *C.char, proof *C.char, vkeyHash *C.char, committedValuesDigest *C.char) *C.char {
	...
	err := monerochan.VerifyGroth16(dataDirString, proofString, vkeyHashString, committedValuesDigestString)
	if err != nil {
		return C.CString(err.Error())
	}
	return nil
...
func TestMain() error {
	...
	var inputs monerochan.WitnessInput
	...
	// Compile the circuit.
	circuit := monerochan.NewCircuit(inputs)
	builder := scs.NewBuilder
	scs, err := frontend.Compile(ecc.BN254.ScalarField(), builder, &circuit)
	if err != nil {
		return err
	}
	fmt.Println("[monerochan] gnark verifier constraints:", scs.GetNbConstraints())
	...
	// Generate witness.
	assignment := monerochan.NewCircuit(inputs)
...
func TestPoseidonBabyBear2() *C.char {
	...
	circuit := monerochan.TestPoseidon2BabyBearCircuit{Input: input, ExpectedOutput: expectedOutput}
	assignment := monerochan.TestPoseidon2BabyBearCircuit{Input: input, ExpectedOutput: expectedOutput}
