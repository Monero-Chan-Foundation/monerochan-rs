...
//! curve.
//!
//! This implementation is specifically for use inside of MONEROCHAN, and internally uses MONEROCHAN's Weierstrass
//! precompiles. Weierstrass precompiles.
//!
//! In summary, MONEROCHAN overrides curve arithmetic entirely, and patches upstream field operations
//! to be more efficient in the VM, such as `sqrt` or `inverse`.

use crate::utils::AffinePoint as MONEROCHANAffinePointTrait;
...
/// The affine point type for MONEROCHAN.
pub mod affine;
pub use affine::AffinePoint;

/// The projective point type for MONEROCHAN.
pub mod projective;
...
/// A [`CurveArithmetic`] extension for MONEROCHAN acceleration.
///
/// Patched crates implement this trait to take advantage of MONEROCHAN-specific acceleration in the zkVM
/// context.
///
    ...
    >,
{
    ...
    /// The underlying [`MONEROCHANAffinePointTrait`] implementation.
    type MONEROCHANAffinePoint: ECDSAPoint;
...
/// Alias trait for the [`MONEROCHANAffinePointTrait`] with 32 byte field elements.
pub trait ECDSAPoint:
    MONEROCHANAffinePointTrait<POINT_LIMBS> + Clone + Copy + Debug + Send + Sync
{
    #[inline]
    fn from(x: &[u8], y: &[u8]) -> Self {
        <Self as MONEROCHANAffinePointTrait<POINT_LIMBS>>::from(x, y)
    }
}

impl<P> ECDSAPoint for P where
    P: MONEROCHANAffinePointTrait<POINT_LIMBS> + Clone + Copy + Debug + Send + Sync
{
}
