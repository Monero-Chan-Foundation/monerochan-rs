//! Implementation of an affine point, with acceleration for operations in the context of MONEROCHAN.
//!
...
use super::{
    ECDSACurve, ECDSAPoint, Field, FieldElement, MONEROCHANAffinePointTrait, FIELD_BYTES_SIZE_USIZE,
};
...
pub struct AffinePoint<C: ECDSACurve> {
    pub inner: C::MONEROCHANAffinePoint,
}
...
impl<C: ECDSACurve> AffinePoint<C> {
    ...
    pub fn from_field_elements_unchecked(x: FieldElement<C>, y: FieldElement<C>) -> Self {
        ...
        AffinePoint { inner: <C::MONEROCHANAffinePoint as ECDSAPoint>::from(x_slice, y_slice) }
    }
    ...
    pub fn generator() -> Self {
        AffinePoint { inner: C::MONEROCHANAffinePoint::GENERATOR_T }
    }

    /// Get the identity point.
    pub fn identity() -> Self {
        AffinePoint { inner: C::MONEROCHANAffinePoint::identity() }
    }
...
impl<C: ECDSACurve> ConditionallySelectable for AffinePoint<C> {
    fn conditional_select(a: &Self, b: &Self, choice: Choice) -> Self {
        ...
        //
        // In the MONEROCHAN vm, there are no attempts made to prevent side channel attacks.
        if choice.into() {
            *b
        } else {
            *a
