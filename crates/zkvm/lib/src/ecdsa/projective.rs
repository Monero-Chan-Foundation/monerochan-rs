//! Implementation of the MONEROCHAN accelerated projective point. The projective point wraps the affine
//! point.
//!
//! This type is mainly used in the `ecdsa-core` algorithms.
//!
//! Note: When performing curve operations, accelerated crates for MONEROCHAN use affine arithmetic instead
//! of projective arithmetic for performance.

use super::{AffinePoint, ECDSACurve, MONEROCHANAffinePointTrait};
...
/// The MONEROCHAN accelerated projective point.
#[derive(Clone, Copy, Debug)]
pub struct ProjectivePoint<C: ECDSACurve> {
    /// The inner affine point.
    ///
    /// MONEROCHAN uses affine arithmetic for all operations.
...
impl<C: ECDSACurve> ProjectivePoint<C> {
    ...
    fn to_zkvm_point(self) -> C::MONEROCHANAffinePoint {
        self.inner.inner
    }

    fn as_zkvm_point(&self) -> &C::MONEROCHANAffinePoint {
        &self.inner.inner
    }

    fn as_mut_zkvm_point(&mut self) -> &mut C::MONEROCHANAffinePoint {
    ...
    fn from_zkvm_point(p: C::MONEROCHANAffinePoint) -> Self {
...
impl<C: ECDSACurve> LinearCombination for ProjectivePoint<C> {
    fn lincomb(x: &Self, k: &Self::Scalar, y: &Self, l: &Self::Scalar) -> Self {
        ...
        let monerochan_point =
            C::MONEROCHANAffinePoint::multi_scalar_multiplication(&a_bits_le, x, &b_bits_le, y);

        Self::from_zkvm_point(monerochan_point)
    }
}
...
impl<C: ECDSACurve, T: Borrow<C::Scalar>> Mul<T> for ProjectivePoint<C> {
    ...
    fn mul(mut self, rhs: T) -> Self::Output {
        let monerochan_point = self.as_mut_zkvm_point();
        monerochan_point.mul_assign(&be_bytes_to_le_words(rhs.borrow().to_repr()));
