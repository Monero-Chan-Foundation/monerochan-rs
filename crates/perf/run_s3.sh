...
aws s3 cp s3://monerochan-testing-suite/$s3_path/program.bin program.bin
aws s3 cp s3://monerochan-testing-suite/$s3_path/stdin.bin stdin.bin
...
# Run monerochan-perf
cargo run -p monerochan-perf --bin monerochan-perf -- --program program.bin --stdin stdin.bin --mode $kind
