...
use reqwest::Client;
use monerochan::install::download_file;
...
#[derive(Parser)]
#[command(name = "install-toolchain", about = "Install the cargo-monerochan toolchain.")]
...
impl InstallToolchainCmd {
    pub fn run(&self) -> Result<()> {
        ...
        // Setup variables.
        let root_dir = home_dir().unwrap().join(".monerochan");
        match fs::read_dir(&root_dir) {
            ...
            Err(_) => println!("No existing ~/.monerochan directory to remove."),
        }
        println!("Successfully cleaned up ~/.monerochan directory.");
        match fs::create_dir_all(&root_dir) {
            Ok(_) => println!("Successfully created ~/.monerochan directory."),
            Err(err) => println!("Failed to create ~/.monerochan directory: {err}"),
        };
