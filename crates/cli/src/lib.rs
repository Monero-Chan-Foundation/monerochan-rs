...
pub const RUSTUP_TOOLCHAIN_NAME: &str = "monerochan";
...
pub const LATEST_SUPPORTED_TOOLCHAIN_VERSION_TAG: &str = "monerochan-rust-1.88.0";

pub const MONEROCHAN_VERSION_MESSAGE: &str =
    concat!("monerochan", " (", env!("VERGEN_GIT_SHA"), " ", env!("VERGEN_BUILD_TIMESTAMP"), ")");
...
pub async fn get_toolchain_download_url(client: &Client, target: String) -> String {
    let all_releases = client
        .get("https://api.github.com/repos/monerochanorg/monerochan-rust/releases")
        .send()
        .await
        .unwrap()
    ...
    let url = format!(
        "https://github.com/monerochanorg/monerochan-rust/releases/download/{LATEST_SUPPORTED_TOOLCHAIN_VERSION_TAG}/rust-toolchain-{target}.tar.gz"
    );
