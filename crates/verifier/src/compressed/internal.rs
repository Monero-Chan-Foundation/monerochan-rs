...
use monerochan_primitives::poseidon2_hash;
...
// NOTE: that all these constants and types are checked by monerochan_prover::tests::monerochan_verifier_valid.
...
/// The vkey merkle tree root's digest. Fixed between MONEROCHAN major versions.
...
/// Unwraps `monerochan_proof: &MONEROCHANProof` into a `&MONEROCHANReduceProof<SC>`, returning
/// `Err(CompressedError::Mode(...))` if the variant is not [`MONEROCHANProof::Compressed`].
/// Then, calls [`verify_monerochan_reduce_proof`] and returns the result.
pub fn verify_monerochan_proof(
    monerochan_proof: &MONEROCHANProof,
    monerochan_public_inputs: &[u8],
    vkey_hash: &[BabyBear; 8],
) -> Result<(), CompressedError> {
    let MONEROCHANProof::Compressed(reduce_proof) = monerochan_proof else {
        return Err(CompressedError::Mode(monerochan_proof.into()));
    };
...
pub fn verify_monerochan_reduce_proof(
    reduce_proof: &MONEROCHANReduceProof<SC>,
    monerochan_public_inputs: &[u8],
    vkey_hash: &[BabyBear; 8],
) -> Result<(), CompressedError> {
    let MONEROCHANReduceProof { vk: compress_vk, proof } = reduce_proof;
    ...
    // Validate the MONEROCHAN public values against the committed digest.
    ...
    if committed_value_digest_bytes.as_slice() != hash_public_inputs(monerochan_public_inputs).as_slice() &&
        committed_value_digest_bytes.as_slice() !=
            hash_public_inputs_with_fn(monerochan_public_inputs, blake3_hash)
    {
    ...
    if public_values.monerochan_vk_digest != *vkey_hash {
        return Err(MachineVerificationError::InvalidPublicValues("monerochan vk hash mismatch").into());
    }
