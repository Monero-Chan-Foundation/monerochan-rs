    ...
    F: Fn(&[u8]) -> [u8; 32],
{
    ...
    // out the first 3 bits. The same logic happens in the MONEROCHAN Ethereum verifier contract.
    result[0] &= 0x1F;
...
/// Formats the monerochan vkey hash and public inputs for use in either the Plonk or Groth16 verifier.
pub fn bn254_public_values(monerochan_vkey_hash: &[u8; 32], monerochan_public_inputs: &[u8]) -> [Fr; 2] {
    let committed_values_digest = hash_public_inputs(monerochan_public_inputs);
    let vkey_hash = Fr::from_slice(&monerochan_vkey_hash[1..]).unwrap();
    let committed_values_digest = Fr::from_slice(&committed_values_digest).unwrap();
    [vkey_hash, committed_values_digest]
}

/// Decodes the monerochan vkey hash from the string from a call to `vk.bytes32`.
pub fn decode_monerochan_vkey_hash(monerochan_vkey_hash: &str) -> Result<[u8; 32], Error> {
    let bytes = hex::decode(&monerochan_vkey_hash[2..]).map_err(|_| Error::InvalidProgramVkeyHash)?;
