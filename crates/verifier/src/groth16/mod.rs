...
use crate::{
    blake3_hash, constants::VK_HASH_PREFIX_LENGTH, decode_monerochan_vkey_hash, error::Error,
...
impl Groth16Verifier {
    /// Verifies an MONEROCHAN Groth16 proof, as generated by the MONEROCHAN SDK.
    ///
    /// # Arguments
    ///
    /// * `proof` - The proof bytes.
    /// * `public_inputs` - The MONEROCHAN public inputs.
    /// * `monerochan_vkey_hash` - The MONEROCHAN vkey hash. This is generated in the following manner:
    ///
    /// ```ignore
    /// use monerochan::ProverClient;
    /// let client = ProverClient::new();
    /// let (pk, vk) = client.setup(ELF);
    /// let monerochan_vkey_hash = vk.bytes32();
    /// ```
    ...
    pub fn verify(
        ...
        //
        // MONEROCHAN prepends the raw Groth16 proof with the first 4 bytes of the groth16 vkey to
        // facilitate this check.
        if groth16_vk_hash != proof[..VK_HASH_PREFIX_LENGTH] {
            return Err(Groth16Error::Groth16VkeyHashMismatch);
        }

        let monerochan_vkey_hash = decode_monerochan_vkey_hash(monerochan_vkey_hash)?;
        ...
        if Self::verify_gnark_proof(
            ...
            &[monerochan_vkey_hash, hash_public_inputs(monerochan_public_inputs)],
            groth16_vk,
        )
        .is_ok()
        {
        ...
        Self::verify_gnark_proof(
            &proof[VK_HASH_PREFIX_LENGTH..],
            &[monerochan_vkey_hash, hash_public_inputs_with_fn(monerochan_public_inputs, blake3_hash)],
            groth16_vk,
        )
    }
    ...
    ///
    /// WARNING: if you're verifying an MONEROCHAN proof, you should use [`verify`] instead.
    /// This is a lower-level verification method that works directly with raw bytes rather than
    /// the MONEROCHAN SDK's data structures.
    ///
    /// # Arguments
    ///
    ...
    /// Verifies an MONEROCHAN compressed Groth16 proof, as generated by the MONEROCHAN SDK.
    ///
    /// # Arguments
    ///
    /// * `proof` - The proof bytes.
