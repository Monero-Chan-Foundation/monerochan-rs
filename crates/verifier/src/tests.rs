...
use monerochan::{install::try_install_circuit_artifacts, HashableKey, ProverClient, MONEROCHANStdin};
use test_artifacts::{
...
fn test_verify_core(#[case] elf: &[u8]) {
    ...
    // Generate the proof.
    let monerochan_proof_with_public_values = client.prove(&pk, &MONEROCHANStdin::new()).core().run().unwrap();
...
fn test_verify_compressed(#[case] elf: &[u8]) {
    ...
    // Generate the proof.
    let monerochan_proof_with_public_values =
        client.prove(&pk, &MONEROCHANStdin::new()).compressed().run().unwrap();
...
fn test_verify_groth16(#[case] elf: &[u8]) {
    ...
    // Generate the proof.
    let monerochan_proof_with_public_values = client.prove(&pk, &MONEROCHANStdin::new()).groth16().run().unwrap();
...
fn test_verify_plonk(#[case] elf: &[u8]) {
    ...
    // Generate the proof.
    let monerochan_proof_with_public_values = client.prove(&pk, &MONEROCHANStdin::new()).plonk().run().unwrap();
...
fn test_groth16_verifier_compressed(#[case] elf: &[u8], #[case] groth16_compressed_elf: &[u8]) {
    ...
    let proof = monerochan_proof_with_public_values.bytes();
    let public_inputs = monerochan_proof_with_public_values.public_values.to_vec();
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "ark")] {
            ...
            use crate::{
                decode_monerochan_vkey_hash, hash_public_inputs, load_ark_groth16_verifying_key_from_bytes,
            ...
            let ark_public_inputs = load_ark_public_inputs_from_bytes(
                &decode_monerochan_vkey_hash(&vkey_hash).unwrap(),
