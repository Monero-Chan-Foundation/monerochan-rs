...
use monerochan_core_executor::{Executor, Program};
...
use monerochan_stark::{baby_bear_poseidon2::MyHash as InnerHash, MONEROCHANCoreOpts, Word};

use crate::InnerSC;

/// Get the MONEROCHAN vkey BabyBear Poseidon2 digest this reduce proof is representing.
pub fn monerochan_vkey_digest_babybear(proof: &MONEROCHANReduceProof<BabyBearPoseidon2Outer>) -> [BabyBear; 8] {
    let proof = &proof.proof;
    let pv: &RecursionPublicValues<BabyBear> = proof.public_values.as_slice().borrow();
    pv.monerochan_vk_digest
}

/// Get the MONEROCHAN vkey Bn Poseidon2 digest this reduce proof is representing.
pub fn monerochan_vkey_digest_bn254(proof: &MONEROCHANReduceProof<BabyBearPoseidon2Outer>) -> Bn254Fr {
    babybears_to_bn254(&monerochan_vkey_digest_babybear(proof))
}
...
pub fn root_public_values_digest(
    ...
    let hash = InnerHash::new(config.perm.clone());
    let input = (*public_values.monerochan_vk_digest())
        .into_iter()
        .chain(
...
pub fn monerochan_committed_values_digest_bn254(
...
pub fn get_cycles(elf: &[u8], stdin: &MONEROCHANStdin) -> u64 {
