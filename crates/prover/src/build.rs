...
use monerochan_recursion_circuit::{
    ...
    machine::{MONEROCHANCompressWitnessValues, MONEROCHANWrapVerifier},
};
...
use monerochan_recursion_gnark_ffi::{Groth16Bn254Prover, PlonkBn254Prover};
use monerochan_stark::{MONEROCHANProverOpts, ShardProof, StarkVerifyingKey};

pub use monerochan_recursion_circuit::witness::{OuterWitness, Witnessable};
pub use monerochan_recursion_core::stark::monerochan_dev_mode;

use crate::{
    utils::{babybear_bytes_to_bn254, babybears_to_bn254, words_to_bytes},
    OuterSC, MONEROCHANProver, WrapAir,
};
...
pub fn try_build_plonk_bn254_artifacts_dev(
    ...
    println!("[monerochan] building plonk bn254 artifacts in development mode");
...
pub fn try_build_groth16_bn254_artifacts_dev(
    ...
    println!("[monerochan] building groth16 bn254 artifacts in development mode");
...
pub fn plonk_bn254_artifacts_dev_dir() -> PathBuf {
    dirs::home_dir().unwrap().join(".monerochan").join("circuits").join("dev")
}
...
pub fn build_constraints_and_witness(
    ...
    let template_input = MONEROCHANCompressWitnessValues {
    ...
    let vkey_hash = babybears_to_bn254(&pv.monerochan_vk_digest);
...
pub fn dummy_proof() -> (StarkVerifyingKey<OuterSC>, ShardProof<OuterSC>) {
    ...
    let prover: MONEROCHANProver = MONEROCHANProver::new();
...
fn build_outer_circuit(template_input: &MONEROCHANCompressWitnessValues<OuterSC>) -> Vec<Constraint> {
    ...
    // Verify the proof.
    MONEROCHANWrapVerifier::verify(&mut builder, &wrap_machine, input);
