...
use crate::shapes::MONEROCHANCompressProgramShape;
use lru::LruCache;
use p3_baby_bear::BabyBear;
use p3_field::{AbstractField, PrimeField, PrimeField32};
use p3_matrix::dense::RowMajorMatrix;
use shapes::MONEROCHANProofShape;
...
use monerochan_core_machine::{
    ...
    utils::{concurrency::TurnBasedSync, MONEROCHANCoreProverError},
};
use monerochan_primitives::hash_deferred_proof;
pub use monerochan_primitives::io::MONEROCHANPublicValues;
use monerochan_recursion_circuit::{
    hash::FieldHasher,
    machine::{
        PublicValuesOutputDigest, MONEROCHANCompressRootVerifierWithVKey, MONEROCHANCompressShape,
        MONEROCHANCompressWithVKeyVerifier, MONEROCHANCompressWithVKeyWitnessValues, MONEROCHANCompressWithVkeyShape,
        MONEROCHANCompressWitnessValues, MONEROCHANDeferredVerifier, MONEROCHANDeferredWitnessValues,
        MONEROCHANMerkleProofWitnessValues, MONEROCHANRecursionShape, MONEROCHANRecursionWitnessValues,
        MONEROCHANRecursiveVerifier,
    },
...
pub use monerochan_recursion_gnark_ffi::proof::{Groth16Bn254Proof, PlonkBn254Proof};
use monerochan_recursion_gnark_ffi::{groth16_bn254::Groth16Bn254Prover, plonk_bn254::PlonkBn254Prover};
use monerochan_stark::{
    baby_bear_poseidon2::BabyBearPoseidon2,
    shape::{OrderedShape, Shape},
    Challenge, MachineProver, MachineProvingKey, MONEROCHANProverOpts, ShardProof, SplitOpts,
    StarkGenericConfig, StarkVerifyingKey, Val, Word, DIGEST_SIZE,
};
use tracing::instrument;

pub use types::*;
use utils::{monerochan_committed_values_digest_bn254, monerochan_vkey_digest_bn254, words_to_bytes};

use components::{CpuProverComponents, MONEROCHANProverComponents};
...
/// docker image.
pub const MONEROCHAN_CIRCUIT_VERSION: &str = include_str!("../MONEROCHAN_VERSION");

/// The configuration for the outer prover.
pub type OuterSC = BabyBearPoseidon2Outer;

pub type DeviceProvingKey<C> = <<C as MONEROCHANProverComponents>::CoreProver as MachineProver<
    BabyBearPoseidon2,
    RiscvAir<BabyBear>,
>>::DeviceProvingKey;
...
/// wrapping.
pub struct MONEROCHANProver<C: MONEROCHANProverComponents = CpuProverComponents> {
    /// The core prover.
    ...
    pub lift_programs_lru: Mutex<LruCache<MONEROCHANRecursionShape, Arc<RecursionProgram<BabyBear>>>>,
    ...
    pub join_programs_map: BTreeMap<MONEROCHANCompressWithVkeyShape, Arc<RecursionProgram<BabyBear>>>,
...
impl<C: MONEROCHANProverComponents> MONEROCHANProver<C> {
    /// Initializes a new [MONEROCHANProver].
    #[instrument(name = "initialize prover", level = "debug", skip_all)]
    pub fn new() -> Self {
        Self::uninitialized()
    }

    /// Creates a new [MONEROCHANProver] with lazily initialized components.
    pub fn uninitialized() -> Self {
        ...
        let program_cache_disabled = env::var("MONEROCHAN_DISABLE_PROGRAM_CACHE")
            .map(|v| v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if !program_cache_disabled {
            if let Some(config) = &recursion_shape_config {
                MONEROCHANProofShape::generate_compress_shapes(config, REDUCE_BATCH_SIZE).for_each(
                    |shape| {
                        let compress_shape = MONEROCHANCompressWithVkeyShape {
                            compress_shape: shape.into(),
                            merkle_tree_height: merkle_tree.height,
                        };
                        let input = MONEROCHANCompressWithVKeyWitnessValues::dummy(
    ...
    pub fn setup(
        &self,
        elf: &[u8],
    ) -> (MONEROCHANProvingKey, DeviceProvingKey<C>, Program, MONEROCHANVerifyingKey) {
    ...
    /// Execute an MONEROCHAN program with the specified inputs.
    ...
    pub fn execute<'a>(
    ...
    ) -> Result<(MONEROCHANPublicValues, [u8; 32], ExecutionReport), ExecutionError> {
        ...
        let (opts, program) = if calculate_gas {
            ...
            (monerochan_stark::MONEROCHANCoreOpts::default(), Program::from(elf).unwrap())
        };
        ...
        Ok((
            MONEROCHANPublicValues::from(&runtime.state.public_values_stream),
    ...
    pub fn prove_core<'a>(
        &'a self,
        pk_d: &<<C as MONEROCHANProverComponents>::CoreProver as MachineProver<
            BabyBearPoseidon2,
            RiscvAir<BabyBear>,
        >>::DeviceProvingKey,
        program: Program,
        ...
        std::thread::scope(|s| {
            ...
            let handle = s.spawn(move || {
                ...
                let gas_calculator = (context.calculate_gas
                    && std::env::var("MONEROCHAN_FORCE_GAS").is_ok())
                .then(
                    || -> Box<dyn FnOnce(&RecordEstimator) -> Result<u64, Box<dyn Error>> + '_> {
                        ...
                        if opts.core_opts == gas::GAS_OPTS {
                            tracing::info!(
                                "The MONEROCHANCoreOpts matches the gas opts, so gas will be consistent."
                            );
                        } else {
                            tracing::warn!(
                                "The MONEROCHANCoreOpts does not match the gas opts. \
                ...
                monerochan_core_machine::utils::prove_core_stream::<_, C::CoreProver>(
                    &self.core_prover,
                    pk,
                    program,
                    stdin,
            ...
            for _ in 0..3 {
                if let Ok((shape, is_complete)) = shape_rx.recv() {
                    let recursion_shape =
                        MONEROCHANRecursionShape { proof_shapes: vec![shape], is_complete };

                    // Only need to compile the recursion program if we're not in the one-shard
                    // case.
                    let compress_shape = MONEROCHANCompressProgramShape::Recursion(recursion_shape);
            ...
            Ok(MONEROCHANCoreProof {
                proof: MONEROCHANCoreProofData(shard_proofs),
                stdin: stdin.clone(),
                public_values,
                cycles,
            })
    ...
    pub fn compress(
        ...
        enum TracesOrInput {
            ...
            CircuitWitness(Box<MONEROCHANCircuitWitness>),
        }
        ...
        let (vk, proof) = thread::scope(|s| {
            ...
            let (input_tx, input_rx) = sync_channel::<(usize, usize, MONEROCHANCircuitWitness, bool)>(
                    ...
                    loop {
                        ...
                        if let Ok((index, height, input, false)) = received {
                            ...
                            .in_scope(|| match input {
                                MONEROCHANCircuitWitness::Core(input) => {
                                    let mut witness_stream = Vec::new();
                                    Witnessable::<InnerConfig>::write(&input, &mut witness_stream);
                                    (self.recursion_program(&input), witness_stream)
                                }
                                MONEROCHANCircuitWitness::Deferred(input) => {
                                    let mut witness_stream = Vec::new();
                                    Witnessable::<InnerConfig>::write(&input, &mut witness_stream);
                                    (self.deferred_program(&input), witness_stream)
                                }
                                MONEROCHANCircuitWitness::Compress(input) => {
                                    let mut witness_stream = Vec::new();
                            ...
                            let record = tracing::debug_span!("execute runtime").in_scope(|| {
                                ...
                                runtime
                                    .run()
                                    .map_err(|e| {
                                        MONEROCHANRecursionProverError::RuntimeError(e.to_string())
                                    })
                                    .unwrap();
                                runtime.record
                            });
                ...
                let handle = s.spawn(move || {
                    ...
                    loop {
                        ...
                        )) = received
                        {
                            ...
                            if let MONEROCHANCircuitWitness::Compress(inner_witness) = witness {
                                let MONEROCHANCompressWitnessValues { vks_and_proofs, is_complete: _ } =
                                    inner_witness;
                    ...
                    loop {
                        ...
                        if let Ok((index, height, vk, proof)) = received {
                            ...
                            let vks_and_proofs = inputs
                                ...
                                .collect::<Vec<_>>();
                            let input = MONEROCHANCircuitWitness::Compress(MONEROCHANCompressWitnessValues {
                                vks_and_proofs,
                                is_complete,
                            });
        ...
        Ok(MONEROCHANReduceProof { vk, proof })
    }
    ...
    pub fn shrink(
        ...
        let MONEROCHANReduceProof { vk: compressed_vk, proof: compressed_proof } = reduced_proof;
        let input = MONEROCHANCompressWitnessValues {
        ...
        runtime.run().map_err(|e| MONEROCHANRecursionProverError::RuntimeError(e.to_string()))?;
        ...
        Ok(MONEROCHANReduceProof { vk: shrink_vk, proof: compress_proof.shard_proofs.pop().unwrap() })
    }
    ...
    pub fn wrap_bn254(
        &self,
        compressed_proof: MONEROCHANReduceProof<InnerSC>,
        opts: MONEROCHANProverOpts,
    ) -> Result<MONEROCHANReduceProof<OuterSC>, MONEROCHANRecursionProverError> {
        let MONEROCHANReduceProof { vk: compressed_vk, proof: compressed_proof } = compressed_proof;
        let input = MONEROCHANCompressWitnessValues {
        ...
        Ok(MONEROCHANReduceProof { vk: wrap_vk, proof: wrap_proof.shard_proofs.pop().unwrap() })
    }
    ...
    pub fn wrap_plonk_bn254(
        &self,
        proof: MONEROCHANReduceProof<OuterSC>,
        build_dir: &Path,
    ) -> PlonkBn254Proof {
        let input = MONEROCHANCompressWitnessValues {
        ...
        let vkey_hash = monerochan_vkey_digest_bn254(&proof);
        let committed_values_digest = monerochan_committed_values_digest_bn254(&proof);
    ...
    pub fn recursion_program(
        &self,
        input: &MONEROCHANRecursionWitnessValues<CoreSC>,
    ) -> Arc<RecursionProgram<BabyBear>> {
        ...
        match program {
            ...
            None => {
                ...
                tracing::debug_span!("verify").in_scope(|| {
                    MONEROCHANRecursiveVerifier::verify(&mut builder, self.core_prover.machine(), input)
                });
                let block =
    ...
    pub fn compress_program(
        &self,
        input: &MONEROCHANCompressWithVKeyWitnessValues<InnerSC>,
    ) -> Arc<RecursionProgram<BabyBear>> {
    ...
    pub fn shrink_program(
        ...
        // Verify the proof.
        MONEROCHANCompressRootVerifierWithVKey::verify(
            &mut builder,
    ...
    pub fn wrap_program(&self) -> Arc<RecursionProgram<BabyBear>> {
        self.wrap_program
            .get_or_init(|| {
                ...
                let input_shape = MONEROCHANCompressShape::from(vec![shrink_shape]);
                let shape = MONEROCHANCompressWithVkeyShape {
                    compress_shape: input_shape,
                    merkle_tree_height: self.recursion_vk_tree.height,
                };
                let dummy_input =
                    MONEROCHANCompressWithVKeyWitnessValues::dummy(self.shrink_prover.machine(), &shape);
    ...
    pub fn deferred_program(
        &self,
        input: &MONEROCHANDeferredWitnessValues<InnerSC>,
    ) -> Arc<RecursionProgram<BabyBear>> {
        // Compile the program.
        ...
        // Verify the proof.
        MONEROCHANDeferredVerifier::verify(
            &mut builder,
            self.compress_prover.machine(),
            input,
            self.vk_verification,
    ...
    pub fn get_recursion_core_inputs(
    ...
    ) -> Vec<MONEROCHANRecursionWitnessValues<CoreSC>> {
        ...
        for (batch_idx, batch) in shard_proofs.chunks(batch_size).enumerate() {
            ...
            core_inputs.push(MONEROCHANRecursionWitnessValues {
                vk: vk.clone(),
    ...
    pub fn get_recursion_deferred_inputs_with_initial_digest<'a>(
        ...
        deferred_proofs: &[MONEROCHANReduceProof<InnerSC>],
        mut deferred_digest: [Val<CoreSC>; 8],
        batch_size: usize,
    ) -> (Vec<MONEROCHANDeferredWitnessValues<InnerSC>>, [BabyBear; 8]) {
        ...
        for batch in deferred_proofs.chunks(batch_size) {
            ...
            let input = MONEROCHANCompressWitnessValues { vks_and_proofs, is_complete: true };
            let input = self.make_merkle_proofs(input);
            let MONEROCHANCompressWithVKeyWitnessValues { compress_val, merkle_val } = input;

            deferred_inputs.push(MONEROCHANDeferredWitnessValues {
                vks_and_proofs: compress_val.vks_and_proofs,
                vk_merkle_data: merkle_val,
                start_reconstruct_deferred_digest: deferred_digest,
                is_complete: false,
                monerochan_vk_digest: vk.hash_babybear(),
                end_pc: vk.pc_start,
    ...
    pub fn get_first_layer_inputs<'a>(
        &'a self,
        vk: &'a MONEROCHANVerifyingKey,
        shard_proofs: &[ShardProof<InnerSC>],
        deferred_proofs: &[MONEROCHANReduceProof<InnerSC>],
        batch_size: usize,
    ) -> Vec<MONEROCHANCircuitWitness> {
        ...
        inputs.extend(deferred_inputs.into_iter().map(MONEROCHANCircuitWitness::Deferred));
        inputs.extend(core_inputs.into_iter().map(MONEROCHANCircuitWitness::Core));
        inputs
    }
    ...
    pub fn make_merkle_proofs(
        &self,
        input: MONEROCHANCompressWitnessValues<CoreSC>,
    ) -> MONEROCHANCompressWithVKeyWitnessValues<CoreSC> {
        ...
        let merkle_val = MONEROCHANMerkleProofWitnessValues {
            root: self.recursion_vk_root,
            values: vk_digest_values,
            vk_merkle_proofs: proofs,
        };

        MONEROCHANCompressWithVKeyWitnessValues { compress_val: input, merkle_val }
    }

    fn check_for_high_cycles(cycles: u64) {
        if cycles > 100_000_000 {
            tracing::warn!(
                "High cycle count detected ({}M cycles)",
                cycles / 1_000_000
            );
        }
    }
...
pub fn compress_program_from_input<C: MONEROCHANProverComponents>(
    config: Option<&RecursionShapeConfig<BabyBear, CompressAir<BabyBear>>>,
    compress_prover: &C::CompressProver,
    vk_verification: bool,
    input: &MONEROCHANCompressWithVKeyWitnessValues<BabyBearPoseidon2>,
    ...
    let input = input.read(&mut builder);
    // Verify the proof.
    MONEROCHANCompressWithVKeyVerifier::verify(
        &mut builder,
        compress_prover.machine(),
        input,
        vk_verification,
...
pub mod tests {
    ...
    use utils::monerochan_vkey_digest_babybear;
    ...
    pub fn test_e2e_prover<C: MONEROCHANProverComponents>(
    ...
    pub fn bench_e2e_prover<C: MONEROCHANProverComponents>(
        prover: &MONEROCHANProver<C>,
        elf: &[u8],
        stdin: MONEROCHANStdin,
        opts: MONEROCHANProverOpts,
    ...
    pub fn run_e2e_prover_with_options<C: MONEROCHANProverComponents>(
        prover: &MONEROCHANProver<C>,
        elf: &[u8],
        stdin: MONEROCHANStdin,
        opts: MONEROCHANProverOpts,
        ...
        if env::var("COLLECT_SHAPES").is_ok() {
            ...
            for proof in core_proof.proof.0.iter() {
                let shape = MONEROCHANProofShape::Recursion(proof.shape());
                shapes.insert(shape);
            }
        ...
        let vk_digest_babybear = monerochan_vkey_digest_babybear(&wrapped_bn254_proof);
        assert_eq!(vk_digest_babybear, vk.hash_babybear());

        tracing::info!("checking vkey hash bn254");
        let vk_digest_bn254 = monerochan_vkey_digest_bn254(&wrapped_bn254_proof);
    ...
    pub fn test_e2e_with_deferred_proofs_prover<C: MONEROCHANProverComponents>(
        opts: MONEROCHANProverOpts,
    ) -> Result<()> {
    ...
    /// on a Mac M2. Note: This test always re-builds the plonk bn254 artifacts, so setting MONEROCHAN_DEV
    /// is not needed.
    #[test]
    #[serial]
    fn test_e2e() -> Result<()> {
        ...
        let prover = MONEROCHANProver::<CpuProverComponents>::new();
        test_e2e_prover::<CpuProverComponents>(&prover, elf, MONEROCHANStdin::default(), opts, Test::All)
    }
    ...
    fn test_e2e_with_deferred_proofs() -> Result<()> {
        setup_logger();
        test_e2e_with_deferred_proofs_prover::<CpuProverComponents>(MONEROCHANProverOpts::auto())
    }

    /// Checks that the constants, types, etc. in monerochan-verifier are valid.
    ///
    ...
    /// - Prepare to use `cargo run --release -p monerochan-prover --bin build_recursion_vks [...]` to run
    ...
    /// - Locate `monerochan_prover::shapes::build_vk_map`.
    ...
    /// - Locate `monerochan_prover::shapes::MONEROCHANProofShape::generate`.
    ...
    #[test]
    fn monerochan_verifier_valid() {
        use monerochan_verifier::compressed::internal::{
