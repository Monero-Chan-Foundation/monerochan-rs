...
use monerochan_core_machine::shape::CoreShapeConfig;
use monerochan_recursion_circuit::machine::{
    MONEROCHANCompressWithVKeyWitnessValues, MONEROCHANDeferredWitnessValues, MONEROCHANRecursionWitnessValues,
};
use monerochan_recursion_core::{
    shape::{RecursionShape, RecursionShapeConfig},
    RecursionProgram,
};
use monerochan_stark::{shape::OrderedShape, MachineProver, DIGEST_SIZE};
use thiserror::Error;

pub use monerochan_recursion_circuit::machine::{
    MONEROCHANCompressWithVkeyShape, MONEROCHANDeferredShape, MONEROCHANRecursionShape,
};

use crate::{components::MONEROCHANProverComponents, CompressAir, HashableKey, MONEROCHANProver, ShrinkAir};

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
pub enum MONEROCHANProofShape {
...
pub enum MONEROCHANCompressProgramShape {
    Recursion(MONEROCHANRecursionShape),
    Compress(MONEROCHANCompressWithVkeyShape),
    Deferred(MONEROCHANDeferredShape),
    Shrink(MONEROCHANCompressWithVkeyShape),
}

impl MONEROCHANCompressProgramShape {
...
pub fn check_shapes<C: MONEROCHANProverComponents>(
    reduce_batch_size: usize,
    no_precompiles: bool,
    num_compiler_workers: usize,
    prover: &mut MONEROCHANProver<C>,
) -> bool {
    let (shape_tx, shape_rx) =
        std::sync::mpsc::sync_channel::<MONEROCHANCompressProgramShape>(num_compiler_workers);
    ...
    let all_maximal_shapes = MONEROCHANProofShape::generate_maximal_shapes(
        core_shape_config,
        recursion_shape_config,
        reduce_batch_size,
        no_precompiles,
    )
    .collect::<BTreeSet<MONEROCHANProofShape>>();
    ...
    let compress_ok = std::thread::scope(|s| {
        ...
        all_maximal_shapes.into_iter().for_each(|program_shape| {
            shape_tx
                .send(MONEROCHANCompressProgramShape::from_proof_shape(program_shape, height))
                .unwrap();
        });
...
pub fn build_vk_map<C: MONEROCHANProverComponents + 'static>(
    reduce_batch_size: usize,
    dummy: bool,
    num_compiler_workers: usize,
    ...
    // Setup the prover.
    let mut prover = MONEROCHANProver::<C>::new();
    ...
    let (vk_set, panic_indices, height) = if dummy {
        ...
        let dummy_set = MONEROCHANProofShape::dummy_vk_map(
            core_shape_config,
            recursion_shape_config,
            reduce_batch_size,
        )
        ...
        let (shape_tx, shape_rx) =
            std::sync::mpsc::sync_channel::<(usize, MONEROCHANCompressProgramShape)>(num_compiler_workers);
        ...
        for shape in
            MONEROCHANProofShape::generate(core_shape_config, recursion_shape_config, reduce_batch_size)
        {
        ...
        std::thread::scope(|s| {
            ...
            for _ in 0..num_compiler_workers {
                ...
                s.spawn(move || {
                    while let Ok((i, shape)) = shape_rx.lock().unwrap().recv() {
                        ...
                        let is_shrink = matches!(shape, MONEROCHANCompressProgramShape::Shrink(_));
                        let prover = prover.clone();
            ...
            subset_shapes
                .clone()
                .into_iter()
                .map(|(i, shape)| (i, MONEROCHANCompressProgramShape::from_proof_shape(shape, height)))
...
pub fn build_vk_map_to_file<C: MONEROCHANProverComponents + 'static>(
    build_dir: PathBuf,
    reduce_batch_size: usize,
    dummy: bool,
    num_compiler_workers: usize,
...
impl MONEROCHANCompressProgramShape {
    pub fn from_proof_shape(shape: MONEROCHANProofShape, height: usize) -> Self {
        match shape {
            MONEROCHANProofShape::Recursion(proof_shape) => Self::Recursion(proof_shape.into()),
            MONEROCHANProofShape::Deferred(proof_shape) => {
                Self::Deferred(MONEROCHANDeferredShape::new(vec![proof_shape].into(), height))
            }
            MONEROCHANProofShape::Compress(proof_shapes) => Self::Compress(MONEROCHANCompressWithVkeyShape {
                compress_shape: proof_shapes.into(),
                merkle_tree_height: height,
            }),
            MONEROCHANProofShape::Shrink(proof_shape) => Self::Shrink(MONEROCHANCompressWithVkeyShape {
...
impl<C: MONEROCHANProverComponents> MONEROCHANProver<C> {
    pub fn program_from_shape(
        &self,
        shape: MONEROCHANCompressProgramShape,
        shrink_shape: Option<RecursionShape>,
    ) -> Arc<RecursionProgram<BabyBear>> {
        match shape {
            MONEROCHANCompressProgramShape::Recursion(shape) => {
                let input = MONEROCHANRecursionWitnessValues::dummy(self.core_prover.machine(), &shape);
                self.recursion_program(&input)
            }
            MONEROCHANCompressProgramShape::Deferred(shape) => {
                let input = MONEROCHANDeferredWitnessValues::dummy(self.compress_prover.machine(), &shape);
                self.deferred_program(&input)
            }
            MONEROCHANCompressProgramShape::Compress(shape) => {
                let input =
                    MONEROCHANCompressWithVKeyWitnessValues::dummy(self.compress_prover.machine(), &shape);
                self.compress_program(&input)
            }
            MONEROCHANCompressProgramShape::Shrink(shape) => {
                let input =
                    MONEROCHANCompressWithVKeyWitnessValues::dummy(self.compress_prover.machine(), &shape);
                self.shrink_program(
