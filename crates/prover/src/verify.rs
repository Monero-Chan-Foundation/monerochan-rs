...
use monerochan_core_executor::subproof::SubproofVerifier;
use monerochan_core_machine::cpu::MAX_CPU_LOG_DEGREE;
use monerochan_primitives::{
    consts::WORD_SIZE,
    io::{blake3_hash, MONEROCHANPublicValues},
};

use monerochan_recursion_circuit::machine::RootPublicValues;
use monerochan_recursion_core::{air::RecursionPublicValues, stark::BabyBearPoseidon2Outer};
use monerochan_recursion_gnark_ffi::{
...
use monerochan_stark::{
    ...
    MachineProof, MachineProver, MachineVerificationError, MONEROCHANReduceProof, StarkGenericConfig,
    Word,
};
use thiserror::Error;

use crate::{
    components::MONEROCHANProverComponents,
    utils::{is_recursion_public_values_valid, is_root_public_values_valid},
    CoreSC, HashableKey, OuterSC, MONEROCHANCoreProofData, MONEROCHANProver, MONEROCHANVerifyingKey,
};
...
pub enum PlonkVerificationError {
    ...
    #[error(
        "the public values in the monerochan proof do not match the public values in the inner plonk bn254 proof"
    )]
    InvalidPublicValues,
}
...
pub enum Groth16VerificationError {
    ...
    InvalidVerificationKey,
    #[error(
        "the public values in the monerochan proof do not match the public values in the inner groth16 bn254 proof"
    )]
    InvalidPublicValues,
}
...
impl<C: MONEROCHANProverComponents> MONEROCHANProver<C> {
    ...
    pub fn verify_compressed(
        ...
        let MONEROCHANReduceProof { vk: compress_vk, proof } = proof;
        ...
        // Verify that the proof is for the monerochan vkey we are expecting.
        let vkey_hash = vk.hash_babybear();
        if public_values.monerochan_vk_digest != vkey_hash {
            return Err(MachineVerificationError::InvalidPublicValues("monerochan vk hash mismatch"));
        }
    ...
    pub fn verify_wrap_bn254(
        &self,
        proof: &MONEROCHANReduceProof<BabyBearPoseidon2Outer>,
        vk: &MONEROCHANVerifyingKey,
...
impl<C: MONEROCHANProverComponents> SubproofVerifier for MONEROCHANProver<C> {
    fn verify_deferred_proof(
        &self,
        proof: &monerochan_core_machine::reduce::MONEROCHANReduceProof<BabyBearPoseidon2>,
        vk: &monerochan_stark::StarkVerifyingKey<BabyBearPoseidon2>,
        vk_hash: [u32; 8],
        ...
        self.verify_compressed(
            &MONEROCHANReduceProof { vk: proof.vk.clone(), proof: proof.proof.clone() },
            &MONEROCHANVerifyingKey { vk: vk.clone() },
        )?;
