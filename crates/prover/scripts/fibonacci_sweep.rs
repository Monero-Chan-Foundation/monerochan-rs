...
fn main() {
    ...
    let env_filter = EnvFilter::try_from_default_env()
        ...
        .add_directive("monerochan_core=off".parse().unwrap());
    ...
    for (shard_size, iterations, batch_size) in iproduct!(shard_sizes, iterations, batch_sizes) {
        ...
        let proof = prover
            .prove_core(&pk, &stdin, MONEROCHANProverOpts::default(), MONEROCHANContext::default())
            .unwrap();
        let leaf_proving_duration = leaf_proving_start.elapsed().as_secs_f64();

        let recursion_proving_start = Instant::now();
        let _ = prover.compress(&vk, proof, vec![], MONEROCHANProverOpts::default());
