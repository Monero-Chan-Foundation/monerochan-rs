...
use monerochan_core_executor::{Executor, Program, RiscvAirId, MONEROCHANContext};
use monerochan_core_machine::{io::MONEROCHANStdin, riscv::RiscvAir, utils::setup_logger};
use monerochan_stark::{shape::Shape, MONEROCHANCoreOpts};
...
fn main() {
    ...
    // Setup the options.
    let mut opts = MONEROCHANCoreOpts { shard_batch_size: 1, ..Default::default() };
    ...
    for s3_path in program_list {
        ...
        let status = std::process::Command::new("aws")
            .args([
                "s3",
                "cp",
                &format!("s3://monerochan-testing-suite/{s3_path}/program.bin"),
                "program.bin",
            ])
            .status()
        ...
        // Download stdin.bin.
        let status = std::process::Command::new("aws")
            .args(["s3", "cp", &format!("s3://monerochan-testing-suite/{s3_path}/stdin.bin"), "stdin.bin"])
            .status()
        ...
        let stdin: MONEROCHANStdin = bincode::deserialize(&stdin).expect("failed to deserialize stdin");
        ...
        for &log_shard_size in args.shard_sizes.iter() {
            ...
            let new_context = MONEROCHANContext::default();
