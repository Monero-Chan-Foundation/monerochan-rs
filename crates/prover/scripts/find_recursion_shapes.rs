...
use monerochan_prover::{
    ...
    shapes::{check_shapes, MONEROCHANProofShape},
    MONEROCHANProver, ShrinkAir, REDUCE_BATCH_SIZE,
};
use monerochan_recursion_core::shape::RecursionShapeConfig;
use monerochan_stark::{shape::OrderedShape, MachineProver};
...
fn main() {
    ...
    let mut prover = MONEROCHANProver::<CpuProverComponents>::new();
    ...
    assert!({
        ...
        catch_unwind(AssertUnwindSafe(|| {
            prover.shrink_prover.setup(&prover.program_from_shape(
                monerochan_prover::shapes::MONEROCHANCompressProgramShape::from_proof_shape(
                    MONEROCHANProofShape::Shrink(OrderedShape {
